...
### 2026-04-17 — Capstone System Implementation + Debug Session

**Summary:** Built complete Capstone Test System per `Capstone_test_plan.md`. Hit and resolved multiple bugs during integration testing.

---

**New files created:**
- `backend/src/models/capstoneSession.model.js` — CapstoneSession + CapstoneQuestionSchema
- `backend/src/controllers/capstone.controller.js` — 4 endpoints: getCapstoneStatus, generateCapstone, submitCapstone, getCapstoneResult
- `backend/src/routes/capstone.route.js` — mounted at `/api/capstone` in `index.js`
- `frontend/src/store/useCapstoneStore.js` — Zustand store: fetchStatus, generateSession, setAnswer, submitCapstone, fetchResult, clearSession
- `frontend/src/components/CapstoneStatusCard.jsx` — 4 states: locked, cooldown, available, passed
- `frontend/src/components/CapstonePage.jsx` — full exam UI, sessionStorage answer persistence
- `frontend/src/components/CapstoneResultPage.jsx` — score hero, per-question breakdown, retake/pass actions

**Modified files:**
- `backend/src/controllers/test.controller.js` — `checkCourseCompletion` fully rewritten (see bugs below)
- `backend/src/controllers/capstone.controller.js` — `getCapstoneStatus` unlock logic fixed (see bugs below)
- `backend/src/controllers/video.controller.js` — `markWatched` extended to save `courseId`
- `backend/src/lib/aiEvaluator.js` — added `generateCapstoneMCQ()`
- `backend/src/index.js` — mounted `/api/capstone` router
- `frontend/src/components/DashboardPage.jsx` — added Final Exams section with `CapstoneStatusCard`, `capstoneRefreshKey` state, `capstone:unlocked` socket listener
- `frontend/src/App.jsx` — added `/capstone/:courseId` and `/capstone/:courseId/result/:sessionId` routes

---

**Bugs found and fixed this session:**

**BUG 1 — `checkCourseCompletion` queried Progress by `courseId` field which didn't exist on Progress docs**
- Symptom: `allTestsPassed` always false despite student passing all tests
- Root cause: `Progress.find({ studentId, courseId })` returned `[]` — Progress schema had no `courseId` field at the time
- Fix: rewrote CCC to fetch videoIds from `Video.find({ courseId })` and `Test.find({ courseId })`, then query Progress by `videoId: { $in: videoIds }`
- File: `backend/src/controllers/test.controller.js`

**BUG 2 — `allVideosWatched` was a placeholder string `'N/A - unsupported flag'`**
- Symptom: `[CCC] flags { allVideosWatched: 'N/A' }` — unlock never triggered
- Fix: implemented real check using `Video.find({ courseId })` → `Progress.find({ studentId, videoId: { $in: ... }, watched: true })`
- File: `backend/src/controllers/test.controller.js`

**BUG 3 — `computeUnlockState` in `getCapstoneStatus` returned false due to Progress docs with `allTestsPassed: false`**
- Symptom: `unlocked: false` even after CCC set `allTestsPassed: true` on test-linked Progress docs
- Root cause: Progress docs for watch-only videos (no test) have `allTestsPassed: false` by default. `computeUnlockState` checked `progressDocs.some(p => p.allTestsPassed === false)` → always found one → returned false
- Fix: replaced with `progressDocs.some(p => p.allTestsPassed === true)` — positive signal check, not negative
- File: `backend/src/controllers/capstone.controller.js`

**BUG 4 — `getCapstoneStatus` queried Progress by `videoId $in testVideoIds` missing watch-only docs**
- Symptom: `capstonePassed` and `unlocked` flags unreliable
- Fix (pending): simplify all Progress queries in both CCC and `getCapstoneStatus` to use `{ studentId, courseId }` directly now that `courseId` is saved on Progress docs
- File: `backend/src/controllers/capstone.controller.js`, `test.controller.js`

**BUG 5 — `markWatched` did not save `courseId` on Progress documents**
- Symptom: Progress docs missing `courseId` field → `Progress.find({ studentId, courseId })` returns empty
- Fix: added `Video.findById(videoId).select("courseId")` lookup in `markWatched`, saves `courseId` on Progress creation
- File: `backend/src/controllers/video.controller.js`

**BUG 6 — Testing done with wrong userId**
- Symptom: all MongoDB queries returning empty — querying `69e12e7ce9c2c4aa68230acb` but actual logged-in user is `69e1c813c4ca80bdbedb1f7a`
- Fix: confirmed via `GET /api/auth/check`. Use `69e1c813c4ca80bdbedb1f7a` for all queries going forward.

---

**Pending before capstone fully working:**

1. Bulk update existing Progress docs in Atlas — add `courseId: 69b6ea3eb9ba212775749252` to all docs where `studentId: 69e1c813c4ca80bdbedb1f7a`
2. Simplify CCC + `getCapstoneStatus` Progress queries to use `{ studentId, courseId }` directly
3. Fix `markWatched` deployed — new Progress docs will have `courseId` going forward
4. Re-watch all videos + retake all tests with correct user to trigger CCC
5. Verify `unlocked: true` from status endpoint
6. Full C3.5 QA checklist pass

---

**Known technical debt added this session:**
